...
from real_traffic_distribution import traffic_distribution
...
def sample_real_distribution(setting):
    d = traffic_distribution()
    pos_array = d.sample(setting.timeStep, setting.schNum)
    return pos_array
...
class sdn_simulator(object):
    ...
    def pkt_generator(self):
        ...
        nextArrivalTime = sample_real_distribution(self.set)
        logging.info("%s packets are generated" % (len(nextArrivalTime)))

        for i in range(self.schNum):
            self.remainPktNum += len(nextArrivalTime[i])
            for x in nextArrivalTime[i]:
                pkt = Packet(x + self.currentTime, i)
                self.sch_queues[i].enqueue(pkt, x + self.currentTime)
